...
def render(
    ...
    try:
        ...
        def _insert_into_head(jinja, insert):
            ...
            if m:
                before = jinja[:m.end(1)]
                head_content = jinja[m.end(1):m.start(3)]
                after = jinja[m.start(3):]
                return before + insert + head_content + after
            return None

        if "<head" in jinja.lower():
            new_jinja = _insert_into_head(jinja, inserts)
