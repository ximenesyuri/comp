...
class Definer(_Definer('Definer', (TypedFuncType,), {})):
    ...
    def __mul__(self, other: 'Definer') -> 'Definer':
        ...
        InstanceFreeDefiner = _FREE_DEFINER_REGISTRY.get('__FreeDefinerInstance__')
        if InstanceFreeDefiner is None:
            from app.mods.factories import FreeDefiner
            InstanceFreeDefiner = FreeDefiner(1)
            _FREE_DEFINER_REGISTRY['__FreeDefinerInstance__'] = InstanceFreeDefiner

        if not isinstance(self, InstanceFreeDefiner):
            raise TypeError(
                f"The left operand of '*' (i.e., '{self.__name__}') must be a Definer with "
                "exactly one free Jinja variable to be used with concat (FreeDefiner(1)).\n"
                f"Its free variables are: {self.free_jinja_vars}"
            )
