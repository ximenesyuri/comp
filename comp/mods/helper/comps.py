...
def _render_inner(obj):
    try:
        ...
        if type(obj) is Text:       return text(obj)
        if type(obj) is Title:      return title(obj)
        if type(obj) is Link:       return link(obj)
        if type(obj) is Image:      return img(obj)
        if type(obj) is Figure:     return figure(obj)
        if type(obj) is Button:     return button(obj)
        if type(obj) is Logo:       return logo(obj)
        if type(obj) is Item:       return item(obj)
        if type(obj) is Unordered:  return unordered(obj)
        if type(obj) is Ordered:    return ordered(obj)
        if type(obj) is Nav:        return nav(obj)
        if type(obj) is Script:     return script(obj)
        if type(obj) is Asset:      return asset(obj)
        if type(obj) is Alpine:     return alpine(obj)
        if type(obj) is Search:     return search(obj)
        if type(obj) is Input:      return input(obj)
        if type(obj) is Div:        return div(obj)
        if type(obj) is Header:     return header(obj)
        if type(obj) is Footer:     return footer(obj)
        if type(obj) is Main:       return main(obj)
        if type(obj) is Body:       return body(obj)
